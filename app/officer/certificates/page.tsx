...
export default function OfficerCertificatesPage() {
  ...
  return (
    <div>
      <PageHeader title="Certificates" description="All certificates issued through the verification process." />
      ...
      {certificates.length === 0 ? (
        <Empty className="border border-dashed border-border rounded-lg py-12">
          ...
          <EmptyDescription>Certificates appear here once applications are approved.</EmptyDescription>
        </Empty>
      ) : (
              ...
              {certificates.map((cert) => (
                <TableRow key={cert.id}>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" render={<Link href={`/officer/certificates/${cert.id}`} />}>
                      View
